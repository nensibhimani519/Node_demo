...
const getPayIdList = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let result = {};
    if (pageNo == 1) {
        let totalRecords = yield customer_pay_id_1.CustomerPayId.find({ $and: [filter] }).countDocuments();
        result.total_records = totalRecords;
    }
    result.pay_id = yield customer_pay_id_1.CustomerPayId.find({ $and: [filter] }).sort(sort).skip(skip).limit(limit).lean();
    res.status(200).json({ message: "The record has fetched successfully.", data: result });
});
const getPayIdInfo = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { error } = validate(req.body);
    if (error)
        throw error;
    let data = {};
    let customerPayId = yield customer_pay_id_1.CustomerPayId.findOne({ _id: req.body._id });
    if (!customerPayId)
        return res.status(404).json({ message: "No record found." });
    data.pay_id = lodash_1.default.assign(customerPayId, lodash_1.default.pick(req.body, ["currency_code"]));
    res.status(200).json({ message: "The reacord has fetch successfully.", data: data });
});
exports.default = { getPayIdList, getPayIdInfo };
